...
class BuckysTradeOffer
{
    ...
    */
    public function addOffer($targetItemID, $offeredItemID)
    {
        ...
        //If the targetItem owner declined this person's item before?
        //if ($this->checkDeclinedOffer($targetItemID, $offeredItemID, null))
        //    return;
    ...
    */
    ...
    /**
    * Filter the itemlist which are possible to make offer for this item. In a word, we are going to remove used items for this targetItem.
    * 
    * @param integer $targetItemID
        ...
        if (count($itemList) > 0) {
            ...
            $query = sprintf('
                    SELECT tOffer.offerID, tOffer.targetItemID, tOffer.offeredItemID, tOffer.createdDate AS offerCreatedDate, tOffer.lastUpdateDate AS offerUpdatedDate, tOffer.status AS offerStatus, 
                        tItem.userID AS targetUserID, tItem.title AS targetTitle, tItem.subtitle AS targetSubtitle, tItem.description AS targetDescription, tItem.images AS targetImages, tItem.createdDate AS targetCreatedDate, tItem.locationID AS targetLocationID, 
                    ...
                    FROM %s AS tOffer 
                        ...
                        LEFT JOIN %s AS tUser ON tUser.userID = oItem.userID                         
            ', TABLE_TRADE_OFFERS, TABLE_TRADE_ITEMS, TABLE_TRADE_ITEMS, TABLE_COUNTRY, TABLE_TRADE_USERS);
    ...
    */
    public function acceptOffer($offerID) {
        ...
        $tradeItemIns->updateItem($offerInfo['targetItemID'], array('status'=>BuckysTradeItem::STATUS_ITEM_TRADED));
        $tradeItemIns->updateItem($offerInfo['offeredItemID'], array('status'=>BuckysTradeItem::STATUS_ITEM_TRADED));
