    ...
    private function _getSearchQuery($qStr, $type=BuckysSearch::SEARCH_TYPE_USER_AND_PAGE, $sort, $isCount = false) {
        ...
        switch($type) {
            case BuckysSearch::SEARCH_TYPE_USER_AND_PAGE:
                ...
                "
                , $selectStr
                , $relavanceUserStr, TABLE_FRIENDS, TABLE_USERS, TABLE_USERS, $whereUserCondStr
                , $relavancePageStr, TABLE_PAGE_FOLLOWERS,TABLE_PAGES, $wherePageCondStr
                , $orderByStr
                );
            ...
            case BuckysSearch::SEARCH_TYPE_USER:
                ...
                if ($isCount)
                    $query = sprintf("SELECT COUNT(u.userID) AS Count FROM %s AS u %s;", TABLE_USERS, $whereUserCondStr);
                else {
                    
                    $query = sprintf("SELECT u.userID AS userID, '' AS pageID, 'user' AS type, %s AS Relevance, CONCAT(u.firstName, ' ', u.lastName) AS PPName, (SELECT COUNT(DISTINCT(fri.userFriendID)) FROM %s AS fri INNER JOIN %s AS friUT ON friUT.userID=fri.userFriendID WHERE fri.userID=u.userID AND fri.status=1 AND friUT.status=1) PPFollowers FROM %s AS u %s %s", $relavanceUserStr, TABLE_FRIENDS, TABLE_USERS, TABLE_USERS, $whereUserCondStr, $orderByStr);
            ...
            case BuckysSearch::SEARCH_TYPE_PAGE:
                ...
                //search page only
                if ($isCount)
                    $query = sprintf("SELECT COUNT(p.pageID) AS Count FROM %s AS p %s;", TABLE_PAGES, $wherePageCondStr);
                else {
