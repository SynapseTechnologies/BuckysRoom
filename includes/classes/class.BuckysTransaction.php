...
class BuckysTransaction
{
    ...
    */
    public function addPaypalTransaction($data)
    {
        ...
        if ($newID) {
            ...
            $userIns->updateUserFields($data['userID'], array('credits'=>$userInfo['credits']));
            
            
            //Update credits activity
            $data = array(
                'receiverID' => $data['userID'],
                'payerID' => BuckysTransaction::PAYPAL_PAYER_ID,
                'activityType' => BuckysTransaction::ACTIVITY_TYPE_DEPOSIT_BY_PAYPAL,
                'amount' => $credits,
                'transactionID' => $newID,
                'receiverBalance' => $userInfo['credits'],
                'payerBalance' => 0,
    ...
    * @return Indexed
    */
    public function getCreditActivities($userID, $page = 1)
    {
        global $db;
        
        $query = $db->prepare("SELECT ca.*, CONCAT(ru.firstName, ' ', ru.lastName) as receiverName, 
                                      CONCAT(pu.firstName, ' ', pu.lastName) as payerName FROM " . TABLE_CREDIT_ACTIVITY . " AS ca " . 
                              "LEFT JOIN " . TABLE_USERS . " AS ru ON ca.receiverID=ru.userID " .
                              "LEFT JOIN " . TABLE_USERS . " AS pu ON ca.payerID=pu.userID " .
                              "WHERE ca.receiverID=%d OR ca.payerID=%d ORDER BY createdDate DESC ", $userID, $userID);
    ...
    * @param Int $userID
    */
    public function getNumOfCreditActivities($userID)
    {
        global $db;
    ...
    * Send Credits
    */
    public function sendCredits($receiverID, $amount)
    {
        ...
        $data = array(
            ...
            'payerID' => $BUCKYS_GLOBALS['user']['userID'],
            'activityType' => BuckysTransaction::ACTIVITY_TYPE_PAYMENT_TO_OTHER,
            'amount' => $amount,
            'transactionID' => BuckysTransaction::NO_TRANSACTION_ID,
            'receiverBalance' => $receiverInfo['credits'] + $amount,
            'payerBalance' => $BUCKYS_GLOBALS['user']['credits'] - $amount,
            'createdDate' => date('Y-m-d H:i:s')
        );
        ...
        BuckysUser::updateUserFields($BUCKYS_GLOBALS['user']['userID'], array('credits' => $balance - $amount));        
        BuckysUser::updateUserFields($receiverInfo['userID'], array('credits' => $receiverInfo['credits'] + $amount));
    ...
    */
    public function useCreditsInTrade($userID, $amount) {
        ...
        $data = array(
            'receiverID' => BuckysTransaction::BUCKYSROOM_ACCOUNT_ID,
            'payerID' => $userID,
            'activityType' => BuckysTransaction::ACTIVITY_TYPE_TRADE_ITEM_ADD,
            'amount' => $amount,
            'transactionID' => BuckysTransaction::NO_TRANSACTION_ID,
            'receiverBalance' => 0,
            'payerBalance' => $userInfo['credits'],
            'createdDate' => date('Y-m-d H:i:s')
        );
