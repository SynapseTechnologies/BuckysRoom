...
class BuckysTradeCategory
{
    ...
    */
    public function addCategory($name, $parentID=0, $status=1)
    {
        ...
        $newID = $db->insertFromArray(TABLE_TRADE_CATEGORIES, array('name' => $name, 'parentID' => $parentID, 'status' => $status));
    ...
    */
    public function getCategoryByName($catName, $status=1)
    {
        ...
        if ($catName == '')
            return;
        if (isset($status))
            $query = sprintf("SELECT * FROM %s WHERE lower(name)=lower('%s') AND status=%d", TABLE_TRADE_CATEGORIES, $catName, $status);
        else
            $query = sprintf("SELECT * FROM %s WHERE lower(name)=lower('%s')", TABLE_TRADE_CATEGORIES , $catName);
    ...
    */
    public function getCategoryByID($catID, $status=1)
    {
        ...
        if (isset($status))
            $query = $db->prepare("SELECT * FROM " . TABLE_TRADE_CATEGORIES . " WHERE catID=%d AND status=%d", $catID, $status);
        else 
