...
class BuckysPrivateMessenger
{
    ...
    public function getUserList($userID)
    {
        ...
        if($settings['messenger_privacy'] == 'all') { 
            ...
            if(buckys_not_null($blocks1))
                $blocks1 = array_keys($blocks1);
            else
                $blocks1 = array();
            
            $blocks = array_unique(array_merge($blocks, $blocks1, array($userID)));
            ...
            if(buckys_not_null($buddys))
                $buddys = array_keys($buddys);
            else
                $buddys = array();
            $buddys[] = 0;
            
            //Get All users except the people on the blocklist
            $query = $db->prepare("SELECT CONCAT(u.firstName, ' ', u.lastName) as name, u.userID, s.sessionID, u.thumbnail, 1 as online, 1 as buddyStatus FROM " . TABLE_SESSIONS . " AS s " 
                                . " LEFT JOIN " . TABLE_USERS . " AS u ON u.userID=s.userID "
                                . " WHERE s.expiry > %s AND u.status = 1 AND u.userID NOT IN (" . implode(', ', $blocks) . ") AND (u.messenger_privacy = 'all' OR u.userID IN (" . implode(', ', $buddys) . "))"
                                . " ORDER BY name "
                                , $expiry);            
            ...
            $query = $db->prepare("SELECT CONCAT(u.firstName, ' ', u.lastName) as name, u.userID, s.sessionID, u.thumbnail,IF(s.sessionID IS NULL, 0, 1) AS online, b.status as buddyStatus, u.messenger_privacy FROM " . TABLE_MESSENGER_BUDDYLIST . " AS b " 
                                . " LEFT JOIN " . TABLE_SESSIONS . " AS s ON s.expiry > %s AND s.userID=b.buddyID "
                                . " LEFT JOIN " . TABLE_USERS . " AS u ON b.buddyID=u.userID "
                                . " WHERE b.userID=%d AND u.status = 1 ORDER BY online DESC, name "
                                , $expiry, $userID, $userID);
            $rows = $db->getResultsArray($query);            
        }
    ...
    * @return String
    */
    public function getUserListHTML($userID, &$uIDs = array())
    {        
    ...
    * @return Array
    */
    public function getBlockLists($userID)
    {
        global $db;
        
        $query = $db->prepare("SELECT CONCAT(u.firstName, ' ', u.lastName) as `name`, u.userID, u.thumbnail FROM " . TABLE_MESSENGER_BLOCKLIST . " as b "
                ." LEFT JOIN " . TABLE_USERS . " as u on u.userID=b.blockedID WHERE b.userID=%d ORDER BY `name`", $userID);
        $rows =  $db->getResultsArray($query);
    ...
    * @return Array
    */
    public function searchNonBlockedUsers($userID, $term)
    {
        global $db;
        
        $query = "SELECT distinct(u.userID), CONCAT(u.firstName, ' ', u.lastName) as fullName FROM " . TABLE_USERS . " as u LEFT JOIN " . TABLE_MESSENGER_BLOCKLIST . " as b ON b.blockedID = u.userID AND b.userID=$userID WHERE b.messengerBlocklistID IS NULL AND u.status = 1 AND (CONCAT(u.firstName, ' ', u.lastName) LIKE '%" . $db->escapeInput($term) . "%') ORDER BY fullName";
    ...
    */
    public function blockUser($userID, $blockedID)
    {
        ...
        //Block User
        if( !$db->insertFromArray(TABLE_MESSENGER_BLOCKLIST, array('userID' => $userID, 'blockedID' => $blockedID, 'blockedDate' => date('Y-m-d H:i:s'))) )
        {
    ...
    * @return Array
    */
    public function openConversationBox($userID, $cUserID)
    {
        global $db;
    ...
    */
    public function closeConversationBox($userID = null)
    {
    ...
    */
    public function saveMessage($userID, $receiverID, $message)
    {
        global $db;
        ...
        //Create New Message
        $newID = $db->insertFromArray(TABLE_MESSENGER_MESSAGES, 
            array('userID' => $userID, 'buddyID' => $receiverID, 'isNew' => 1, 'messageType' => 0, 'message' => $message, 'createdDate' => $now)
        );
        if(!$newID)
            return $db->getLastError();
        
        //Create New Message
        $newID1 = $db->insertFromArray(TABLE_MESSENGER_MESSAGES,             
            array('userID' => $receiverID, 'buddyID' => $userID, 'isNew' => 1, 'messageType' => 1, 'message' => $message, 'createdDate' => $now)
        );
        if(!$newID1)
            return $db->getLastError();
    ...
    */
    public function getMessages($userID, $buddyID, $type  = 'new')
    {
        ...
        $query = "SELECT m.*, CONCAT(u.firstName, ' ', u.lastName) as fullName, u.userID, u.thumbnail FROM " . TABLE_MESSENGER_MESSAGES . " as m " .
                              " LEFT JOIN " . TABLE_USERS . " as u ON u.userID=m.buddyID " .
    ...
    * @param Int $sID
    */
    public function clearHistory($userID, $sID)
    {
        global $db;
