...
//        newRow.children('label').html($(this).attr('data-label') + (idx + 1) + ":");
        ...
        newRow.find('.visibility_options label:eq(0) input').attr({'id': $(this).attr('data-id') + '_visibility' + idx + '_public', 'name': $(this).attr('data-id') + '_visibility' + idx});
        newRow.find('.visibility_options label:eq(1)').attr('for', $(this).attr('data-id') + '_visibility' + idx + '_private');
        newRow.find('.visibility_options label:eq(1) input').attr({'id': $(this).attr('data-id') + '_visibility' + idx + '_private', 'name': $(this).attr('data-id') + '_visibility' + idx});            
...
//            $(this).children('label').html(label + (idx + 1) + ":");
            ...
            $(this).find('.visibility_options label:eq(0) input').attr({'id': prefix + '_visibility' + idx + '_public', 'name': prefix + 'visibility' + idx});
            $(this).find('.visibility_options label:eq(1)').attr('for', prefix + '_visibility' + idx + '_private');
            $(this).find('.visibility_options label:eq(1) input').attr({'id': prefix + '_visibility' + idx + '_private', 'name': prefix + 'visibility' + idx});            
        })    
    }
    ...
    $('#messengerform').submit(function(){
        ...
        if(isCompleted)
        {
            ...
            var data = {username : acctName, type : acctType, visibility : acctVisibility, action: 'save_messenger', userID: form.find('input[name="userID"]').val()};
    ...
    $('#educationform').submit(function(){
        ...
        if(isCompleted)
        {
            ...
            var data = {schoolname : schoolName, from : from, to : to, visibility : schoolVisibility, action: 'save_education', userID: form.find('input[name="userID"]').val()};
            
            $.ajax({
                type: 'post',
                data: data,
                url: '/info_education.php',
                success: function(rsp){
                    if(rsp == 'Success')
                    {
                        form.find('.input-error').removeClass('input-error');
    ...
    $('#employmentform').submit(function(){
        ...
        if(isCompleted)
        {
            ...
            var data = {employer : employer, from : from, to : to, visibility : schoolVisibility, action: 'save_employment', userID: form.find('input[name="userID"]').val()};
            
            $.ajax({
                type: 'post',
                data: data,
                url: '/info_employment.php',
                success: function(rsp){
                    if(rsp == 'Success')
                    {
                        form.find('.input-error').removeClass('input-error');
    ...
    $('#linksform').submit(function(){
        ...
        if(isCompleted)
        {
            ...
            var data = {title : title, url : url, visibility : visibility, action: 'save_links', userID: form.find('input[name="userID"]').val()};
