...
(function($){
    ...
    $('#tradeEditForm').submit(function(){
        ...
        if(!isValid)
        {
            showMessage($('#tradeEditForm'), 'Please complete the fields in red.', true);
        }else{
            
            //SUBMIT HERE
            $.ajax({
                url: '/trade/process.php',
                data: {action: $("#actionName").val(), 
                       type: $("#actionType").val(),
                       itemID: $("#itemID").val(),
                       title: $("#title").val(),
                ...
                success: function(data)
                {
                    ...
                    if ($("#actionName").val() == 'addTradeItem') {
                        ...
                        setTimeout(function() {document.location.href="/trade/additem.php"}, 3000);
                        
                    }
                    
                    if ($("#actionType").val() == 'relist') {
                    
                        setTimeout(function() {document.location.href="/trade/available.php"}, 3000);
                    }
