...
function validateShippingInfoForm() {
    var requiredFields = ['shippingAddress', 'shippingCity', 'shippingState', 'shippingZip', 'shippingCountryID'];
    var msgMapping = ['Address', 'City', 'State', 'Zip', 'Country']
    
    for (var idx = 0; idx < requiredFields.length; idx++) {
        if (document.getElementById(requiredFields[idx]).value == '') {
            alert("Please input " + msgMapping[idx] + ".");
            document.getElementById(requiredFields[idx]).focus();
            return false;
        }
        ...
        $("#remove_declined_offers").click(function() {
            ...
            if (selectedOffers.length == 0) {
                ...
                if (confirm('Are you sure to remove selected declined offer(s)?') == true) {
                    $.ajax({
                        url: '/trade/process.php',
                        data: {action: 'removeDeclinedOffers', offerIDs: selectedOffers.join(','), type: $("#offer_declined_type").val()},
                        type: 'post',
                        success: function(rsp)
                        {
                            var responseObj = $.parseJSON(rsp);
...
(function($){
    ...
    $(document).on('click', '.edit-tracking-number-box input[type="button"]', function() {
        ...
        if (newTrackingNumber != '') {
            ...
            $.ajax({
                url: '/trade/process.php',
                data: {action: 'saveTrackingNumber', tradeID: currentTradeID, trackingNo: newTrackingNumber},
                type: 'post',
                success: function(rsp)
                {
                    var responseObj = $.parseJSON(rsp);
                    if (responseObj.success == 1) {
                        $("#my_tracking_number_" + currentTradeID).html(newTrackingNumber);
    ...
    $(document).on('click', '.edit-feedback-box input[type="button"]', function() {
        ...
        $.ajax({
            url: '/trade/process.php',
            data: {action: 'saveFeedback', tradeID: currentTradeID, score: score, feedback: $(".edit-feedback-box textarea").val()},
            type: 'post',
            success: function(rsp)
            {
                var responseObj = $.parseJSON(rsp);
