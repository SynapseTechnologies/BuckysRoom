...
if( isset($_POST['action']) )
{
    ...
    {       
        ...
        {
            $newUserHTML = BuckysPrivateMessenger::getUserListHTML($userID);
            render_result_xml( array('status' => 'success', 'id' => $result['userID'], 'name' => $result['firstName'] . " " . $result['lastName'], 'icon' => BuckysUser::getProfileIcon($result), 'html' => $newUserHTML) );
        }else{
    ...
    if($_POST['action'] == 'open-conversation')
    {
        ...
        if( !buckys_not_null($cUserID) || !buckys_not_null($cUserID) || !buckys_check_id_encrypted($cUserID, $cUserIDHash))
        {
            ...
            if( is_array($result) ) //Successfully Added
            {
                ...
                render_result_xml( array('status' => 'success', 'id' => $result['userID'], 'encrypted'=>buckys_encrypt_id($result['userID']), 'name' => $result['firstName'] . " " . $result['lastName'], 'icon' => BuckysUser::getProfileIcon($result), 'content' => $history) );
            }else if($result === true){ //Already exist
                render_result_xml( array('status' => 'exist', 'id' => $result['userID'], 'encrypted'=>buckys_encrypt_id($result['userID']), 'name' => $result['firstName'] . " " . $result['lastName'], 'icon' => BuckysUser::getProfileIcon($result)) );
            }else{
                render_result_xml( array('status' => 'error', 'message' => $result) );
            }
        }
    ...
    if($_POST['action'] == 'send-message')
    {
        ...
        {
            render_result_xml( array('status' => 'success', 'message' => BuckysPrivateMessenger::getMessagesHTML($userID, $receiverID, 'new')) );
        }else{
            render_result_xml( array('status' => 'error', 'message' => $result) );
            exit;
        }
    ...
    if($_POST['action'] == 'clear-history')
    {
        ...
        if($result === true) //Success
        {
            ...
            render_result_xml( array('status' => 'error') );            
        }
        exit;
    }
...
<!--            <a href="#" class="minimize_box_link">&nbsp;</a>-->
        ...
        <div class="below_chat_user_list <?php if($messengerSettings['messenger_privacy'] == 'all'){ ?>add-user-to-buddylist-hidden<?php } ?>" id="add-user-to-buddylist">
...
<!--                <div id="selected-users-list"></div>-->
    ...
    <?php
        ...
        $convList = isset($_SESSION['converation_list']) ? $_SESSION['converation_list'] : array();
    ?>
    <div id="private_messenger_conversation_wrap" <?php if(!buckys_not_null($convList)){?>style="display: none;"<?php } ?>>                            
        <div class="box_nav_row">
            <a href="#" class="close_box_link">&nbsp;</a>
            <a href="#" class="minimize_box_link">&nbsp;</a>            
            ...
            <div id="private_messenger_opened_chats"> 
                <?php                     
                    foreach($convList as $i=>$uID)
                    {
                        ...
                        ?><a href="#" data-id="<?php echo $uID?>" <?php if($i == 0){?>class="actived"<?php } ?> data-encrypted="<?php echo buckys_encrypt_id($uID)?>"><?php echo $tUInfo['firstName'] . " " . $tUInfo['lastName'] ?> <span title="close" class="close-conversation">X</span></a><?php
                    }
                ?>                         
            </div>                    
        </div>        
        <div id="private_messenger_conversation_rgt">
            <?php 
                foreach($convList as $i=>$uID)
                {
                    $tUInfo = BuckysUser::getUserBasicInfo($uID);
            ?>
                <div class="private_messenger_conversation_contr" <?php if($i > 0){?>style="display: none;"<?php } ?> id="private_messenger_conversation_contr<?php echo $uID?>">
                    <?php
