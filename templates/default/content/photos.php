    ...
    <section id="right_side">
        <div class="info-box" id="photos-box">
            <?php if ($view['photo_type'] == 'profile'):?>
                <h3>View All Photos <a href="/profile.php?user=<?php echo $userData['userID']?>" class="view-all">(back to profile)</a></h3>
            <?php elseif ($view['photo_type'] == 'page'): ?>
                <h3>View All Photos <a href="/page.php?pid=<?php echo $pageData['pageID']?>" class="view-all">(back to page)</a></h3>
            <?php endif;?>
            
            <?php if($albums && count($albums) > 0) {?>
            <section id="albums-nav"> 
                <form name="aform" method="get" action="/photos.php" style="padding-top:0px;">
                    <input type="hidden" name="user" value="<?php echo $profileID?>" />
                Select Album:                 
                <select class="select" name="albumID" onchange="document.aform.submit()">
            ...
            <div class="clear"></div>
            <?php if ($view['photo_type'] == 'profile'):?>
                <div id="more-stream" data-page="photo" data-user-id="<?php echo $profileID?>" data-album-id=<?php echo isset($_GET['albumID']) ? $_GET['albumID'] : '' ?>><img src="<?php echo DIR_WS_IMAGE?>loading1.gif" height="15" /></div>
            <?php elseif ($view['photo_type'] == 'page'):?>
                <div id="more-stream" data-page="page-photo" data-page-id="<?php echo $pageData['pageID']?>"><img src="<?php echo DIR_WS_IMAGE?>loading1.gif" height="15" /></div>
            <?php endif;?>
        </div>
