...
<section id="main_section">    
    ...
    <section id="right_side">
        <section id="right_side_padding" class="user-info-section">
            ...
            <form method="post" id="photoslist" action="/photo_manage.php" name="aform" class="user-info" style="padding-top:0px;">                
                <?php render_result_messages() ?>
				<?php $pagination->renderPaginate('/photo_manage.php?' . (!$albumID ? '' : ('albumID=' . $albumID . "&")), count($photos)); ?>
                <?php if($albums && count($albums) > 0) {?>
                <section id="albums-nav" style="text-align:right; margin-top:5px; padding-bottom:10px; border-bottom:1px solid #cccccc;">
                    Select Album:                 
                    <select class="select" name="albumID" onchange="document.location.href='/photo_manage.php?albumID=' + this.value">
                        <option value="">All</option>
                        <?php foreach($albums as $row){ ?>
                        <option value="<?php echo $row['albumID']?>" <?php echo $row['albumID'] == $_GET['albumID'] ? 'selected="selected"' : '' ?>><?php echo $row['name']?></option>
                        <?php } ?>
                    </select>
                </section>
                <?php } ?>
                <div class="table">
                    ...
                    <div class="tr <?php echo $i == count($photos) - 1 ? 'noborder' : ''?>">
                        ...
                        <div class="td td-img">
                            ...
                            <img src="<?php echo DIR_WS_PHOTO?>users/<?php echo $row['poster']?>/resized/<?php echo $row['image']?>" />
                            <?php }else{ ?>
                            <img src="<?php echo DIR_WS_PHOTO?>users/<?php echo $row['poster']?>/thumbnail/<?php echo $row['image']?>" />
                            <?php } ?>
                            </a>
                        </div>
                        ...
                        <div class="td td-action">
                            <br />
                            <a href="/photo_edit.php?photoID=<?php echo $row['postID']?>">Edit Photo</a><br />
                            <?php if($row['image'] == $userData['thumbnail']) { ?>
                              <a href="/photo_manage.php?action=remove-profile-photo&photoID=<?php echo $row['postID']?>" class="remove-link">Remove from Profile Photo</a><br />
                            <?php } else { ?>
                              <a href="/photo_manage.php?action=set-profile-photo&photoID=<?php echo $row['postID']?>">Set as Profile Photo</a><br />
                            <?php } ?>
                            <?php //if($row['image'] != $userData['thumbnail']) { ?>
